        ...
        <div class="demo-charts  mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid" style="background-color: #01aba8;">
          ...
          <div class="mdl-card__supporting-text mdl-color-text--blue-grey-90">
             ...
             <?php
              ...
              $resultado=$EstadisticasController->ComerciosCargados();
              while ($muestra = mysqli_fetch_array($resultado)) {?>
              <h3>Total de comercios: <?php echo  $muestra['ComerciosCargados'];?></h3> <?php }?>
             
             <?php  $resultado=$EstadisticasController->ComerciosHabilitados(); 
              while ($muestra = mysqli_fetch_array($resultado)) {?>
              <h3>Comercios habilitados: <?php echo  $muestra['ComerciosHabilitados'];?></h3> <?php }?>
              
              <?php  $resultado=$EstadisticasController->ComerciosInhabilitados(); 
              while ($muestra = mysqli_fetch_array($resultado)) {?>
              <h3>Comercios inhabilitados: <?php echo  $muestra['ComerciosInhabilitados'];?></h3> <?php }?>
            

  
  <?php  $resultado=$EstadisticasController->ArticulosTotales(); 
              while ($muestra = mysqli_fetch_array($resultado)) {?>
              <h3>Total de Articulos: <?php echo  $muestra['ArticulosTotales'];?></h3> <?php }?>
              


              <?php  $resultado=$EstadisticasController->ArticulosInhabilitados(); 
              while ($muestra = mysqli_fetch_array($resultado)) {?>
              <h3>Articulos inhabilitados: <?php echo  $muestra['ArticulosInhabilitados'];?></h3> <?php }?>
             
             <?php  $resultado=$EstadisticasController->ArticulosHabilitados(); 
              while ($muestra = mysqli_fetch_array($resultado)) {?>
